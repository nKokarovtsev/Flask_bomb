...
def index():
    ...
    timeless = request.form.get('left') #Берем данные (из формы)
    if timeless:
    ...
    try:
        ...
        abort(503) #если Postgres сервер недоступен, ошибка 
    try:
        ...
        idk = []
        tim = [] 
        temp = [] #ненужный параметр убрать
        hum = [] #ненужный параметр убрать
        co = [] #ненужный параметр убрать
        mass = []
        flewin = []
        flewout = []
        delux = [] #ненужный параметр убрать
        mic = [] #ненужный параметр убрать
        
        for column in data:
            idk.append(int(column["idcode"]))
            tim.append(str(column["timemes"])[:-7]) #time
            temp.append(column["temperature"]) #ненужный параметр убрать
            hum.append(column["humidity"]) #ненужный параметр убрать
            co.append(column["co2"]) #ненужный параметр убрать
            mass.append(column["mass"])
            flewin.append(column["flewin"])
            flewout.append(column["flewout"])
            delux.append(round(abs(column["luxin"]-column["luxout"]),2)) #Подсчет разницы света, ненужный параметр убрать
            mic.append(round(float(column["micro"]["Micro"]),2)) #ненужный параметр убрать
...
def create():
    if request.method == 'POST':
        try:
            if "application/json" in request.headers["Content-Type"]:
                ...
                dots = 0
                correct = True
                for symb in str((data['Micro'])).replace("'", ""): #Проверка корректности данных с микрофона
                    if not (symb.isdigit()):
                        if (symb == '.'):
                            dots += 1
                        if (symb != '.' or dots >1):
                ...
                if (correct == True):                    
                    ...
                    Flewin = int(data['flewin'])
                    Flewout = int(data['flewout'])
                    CO2 = float(data['co2'])
    ...
    #отправка в форму с браузера
            ...
            abort(400) #Ошибка 400

        #cur = con.execute()
        #cur.execute(

        try: #устанавливаем коннект с базой
            url = URL.create(
            drivername = "postgresql", 
            username = "postgres",
            password = "3005",
        ...
        with engine.connect() as conn: #Отправляем данные в бд
            conn.execute(
